...
.pl-sala{position:relative;height:470px;max-width:1000px;margin:0 auto}
...
(function(){
  ...
  var randuri=10, total=O.length, coloane=Math.ceil(total/randuri);
  var d=Math.max(9,Math.min(17,Math.round(lat/(coloane*1.35))));
  var cx=lat/2, cy=d*1.6, r0=inalt*0.16;
  var dr=(inalt-cy-d*1.6-r0)/(randuri-1);

  var poz=[], k=0;
  for(var c=0;c<coloane && k<total;c++){
    var t=coloane===1?0.5:c/(coloane-1);
    var a=t*Math.PI;                       // 0 = stânga, π = dreapta
    for(var rr=randuri-1;rr>=0 && k<total;rr--){   // de JOS (rândul exterior) în SUS
      var r=r0+rr*dr;
