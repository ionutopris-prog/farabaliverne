...
STIL = '''
.pl-sala.pl-filtrat .pl-scaun:not(.pl-marcat){opacity:.28}
.pl-scaun.pl-marcat{transform:scale(1.75);z-index:4;
  box-shadow:0 0 0 2px var(--card),0 0 0 3.5px var(--ink),0 3px 8px rgba(34,31,27,.25)}
.pl-scaun.pl-marcat:hover,.pl-scaun.pl-marcat.pl-sel{transform:scale(2.1)}
...
(function(){
  ...
  function marcheaza(nr){
    // Bilele celor care te reprezintă rămân în relief până ştergi căutarea.
    // Restul se estompează, ca ochiul să meargă direct la ele — nu le ascundem,
    // doar le dăm la o parte.
    for(var i=0;i<poz.length;i++){
      var al=(nr!==null && String(O[i][6])===String(nr));
      poz[i].classList.toggle("pl-marcat",al);
    }
    sala.classList.toggle("pl-filtrat",nr!==null);
  }
  ...
  function cautaNume(t){
    ...
    rez.hidden=false; marcheaza(null); return true;
  }
  function cauta(){
    var t=curat(q.value);
    if(t.length<2){rez.hidden=true;marcheaza(null);return;}
    if(!LOC){
